...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/7scenes.yaml", help="Path to YAML config file")
    parser.add_argument("--dataset_folder", type=str, required=True, help="Path to the dataset folder, e.g. '/path/to/replica'")
    ...
    from vista_slam.eval.eval_recon import eval_recon_from_saved_data
    from vista_slam.eval.eval_traj import full_traj_eval
    from vista_slam.datasets.slam_replica import SLAM_Replica
    ...
    for scene_name in scene_names:
        dataset = SLAM_Replica(f"{cfg.dataset_folder}/{scene_name}", resolution=(224,224))
        stride = cfg.stride
        slam = OnlineSLAM(
        ...
        last = len(dataset)
        inputs = list(range(0,last,stride))

        first = True
        t_num = 0
        gt_poses = []
        if len(inputs) > 400:
            inputs = list(torch.linspace(0, last - 1, steps=400).long())
        ...
        print(f"Evaluating reconstruction ...", color=FontColor.EVAL)
        rmse_acc, rmse_comp, chamfer_dist, gt_pcd, pcd_est = eval_recon_from_saved_data(f"{output_folder}/{dataset_name}_{scene_name}", rel_est_gt=[r_a, t_a, s])
        print(f"Results for {scene_name}: RMSE Accuracy: {rmse_acc:.4f}, RMSE Completeness: {rmse_comp:.4f}, Chamfer Distance: {chamfer_dist:.4f}", color=FontColor.EVAL)
        acc.append(rmse_acc)
        comp.append(rmse_comp)
        chamfer.append(chamfer_dist)
        output_str += "#"*10+"recon evaluation"+"#"*9+"\n"
        output_str += f"RMSE Accuracy: {rmse_acc:.4f}\n" 
        output_str += f"RMSE Completeness: {rmse_comp:.4f}\n"
        output_str += f"RMSE Chamfer Distance: {chamfer_dist:.4f}\n"
        output_str += "#"*35 + "\n"
    ...
    for i in range(len(scene_names)):
        ...
        print(f"acc: {acc[i]}", color=FontColor.EVAL)
        print(f"comp: {comp[i]}", color=FontColor.EVAL)
        print(f"chamfer: {chamfer[i]}", color=FontColor.EVAL)

    print()
    print("avg:", color=FontColor.EVAL)
    print(f"traj: {sum(traj_error)/len(traj_error)}", color=FontColor.EVAL)
    print(f"acc: {sum(acc)/len(acc)}", color=FontColor.EVAL)
    print(f"comp: {sum(comp)/len(comp)}", color=FontColor.EVAL)
    print(f"chamfer: {sum(chamfer)/len(chamfer)}", color=FontColor.EVAL)
