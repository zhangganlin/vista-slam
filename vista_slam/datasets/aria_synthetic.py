...
from ..utils.geometry import depthmap_to_camera_coordinates_ARIA


class AriaSynthetic(BaseViewGraphDataset):
    def __init__(self,  
                 ...
                 neighbor_range=10,  # sample from [i-neighbor_range, i+neighbor_range]
        ...
        # self.scene_names.remove('scene0622_00')
        # self.scene_names.remove('scene0237_00')
        # self.scene_names.remove('scene0204_02')
        # self.scene_names.remove('scene0585_01')
        # self.scene_names.remove('scene0044_02')
        # self.scene_names.remove('scene0522_00')
        # self.scene_names.remove('scene0039_00')
        # self.scene_names.remove('scene0143_02')
        # self.scene_names.remove('scene0355_01')
        # self.scene_names.remove('scene0639_00')
    ...
    def _read_view(self, data_dir, view_name, intri, resolution, rng):
        try:
            camera_pose = np.loadtxt(osp.join(data_dir, "pose", f"{view_name}.txt")).astype(np.float32)
            if not np.isfinite(camera_pose).all() or camera_pose.shape != (4, 4):
                # print(data_dir,view_name, "pose invalid")
                return False, None
            # Load RGB image
            rgb_image = imread_cv2(osp.join(data_dir, "color", f"{view_name}.jpg"))
            ...
            # Load depthmap
            depthmap = imread_cv2(osp.join(data_dir, "depth", f"{view_name}.png"), cv2.IMREAD_UNCHANGED)
        except:
            return False, None
        depthmap = depthmap.astype(np.float32) / 1000
        depthmap[~np.isfinite(depthmap)] = 0  # invalid
        depthmap[depthmap > 20] = 0
        ...
        pts3d_cam, valid_mask = depthmap_to_camera_coordinates_ARIA(view['depthmap'], view['camera_intrinsics'])
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        intri_path = osp.join(self.sensor_data_root, scene_id, 'intrinsic/intrinsic_color.txt')
