...
class ScanNet(BaseViewGraphDataset):
    def __init__(self,  
    ...
    def _read_view(self, data_dir, view_name, intri, resolution, rng):
        try:
            ...
            if not np.isfinite(camera_pose).all():
                ...
                return False, None
            # Load RGB image
            rgb_image = imread_cv2(osp.join(data_dir, f"{view_name}.color.jpg"))
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        intri_path = osp.join(self.sensor_data_root, scene_id, 'intrinsic/intrinsic_depth.txt')
        intri = np.loadtxt(intri_path).astype(np.float32)[:3, :3]

        data_path = osp.join(self.sensor_data_root, scene_id, 'sensor_data')
