...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None, max_iter=None):
        ...
        if torch.cuda.is_available():
            log_msg.append('max mem: {memory:.2f} GB')
...
def _get_num_layer_for_vit(var_name, enc_depth, dec_depth):
    if var_name in ("cls_token", "mask_token", "pos_embed", "global_tokens"):
        ...
        return enc_depth
    elif var_name.startswith('dec_block'):
        layer_id = int(var_name.split('.')[1])
...
def get_parameter_groups(model, weight_decay, layer_decay=1.0, skip_list=(), no_lr_scale_list=[], print_info=False):
